...
;                    :end (make-instance 'polar :radius 3 :angle 4))
; 					(make-instance 'line
;                    :start (make-instance 'cart :x 8 :y 0)
;                    :end (make-instance 'polar :radius 3 :angle 9))
; 					(make-instance 'line
;                    :start (make-instance 'cart :x 1 :y 1)
;                    :end (make-instance 'cart :x 4 :y 3))
; 					(make-instance 'line
;                    :start (make-instance 'cart :x 1 :y 2)
;                    :end (make-instance 'cart :x 3 :y 1))
; 					))
...
; (vector-product (nth 2 lines) (nth 3 lines))

; (vector-product (nth 0 lines) (nth 1 lines))
...
; 			(list (+ cx (/ (* (- dx cx) (abs z1)) (abs (- z2 z1)))) (+ cy (/ (* (- dy cy) (abs z1)) (abs (- z2 z1))))))
; 		  NIL)))
...
(defun line-intersections (lines)
  (let ((res (list)))
	  (loop for i from 0 below (- (length lines) 1) do
		(loop for j from (+ i 1) below (length lines) do 
			  (print (list i j))))))
